...
lorentz_spec <- function(w){w <- w*sr; 1000/((w^2 - (w0)^2)^2 + (2*0.5)^2*w^2)}
...
(ep_data %>% 
  ggplot(aes(x = w, y = log(expected_spec))) +
  geom_line() +
  geom_line(aes(y = log(th_spec)), color = "red") +
  geom_point(data = tibble(w = (0:(sr/2-1))/sr, fcamp = map_dbl(w, exp_spec, lorentz_spec)),
             aes(x = w, y = log(fcamp))) +
    geom_point(data = tibble(w = (0:(srn/2-1))/srn, fcamp = map_dbl(w, exp_spec, lorentz_spec)),
             aes(x = w, y = log(fcamp)), color = "blue", shape = "x")
) %>% ggplotly()
