...
sr <- 10
fejer <- function(w, N){if_else(w==0, N, 1/N*(sin(pi*w*N)/sin(pi*w))^2)}
#spec <- function(w){w <- w*sr; 1000/((w^2 - 10^2)^2 + (2*0.5)^2*w^2)}
spec <- function(w){
  if_else(between(w, 0.1, 0.11), 
          1000, 
          if_else(between(w, 0.3, 0.31), 20, 0)
          )}
ep <- function(l){
  integrand <- function(w){spec(w)*fejer(l-w, sr)}
  integrate(Vectorize(integrand), -.5, .5)$value
}
ep_data <- 
  tibble(
    w = seq(0, 0.5, by = .005),
