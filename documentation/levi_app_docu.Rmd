...
Ein gedämpfter harmonischer Oszillator lässt sich durch folgende Gleichung 
beschreiben:
...
summary_plot <- function(sig_data, spec_data, freq_range, sample_rate = 400, f0 = 30, d = 0.5){
  ...
  if (!is.null(fitted_params)) {
    ...
    fft_plot <-
      ...
      labs(subtitle = (
        ...
        title = str_glue("Lorentz: {round(estimate[2], 2)} Hz;  d = {round(estimate[3], 1)/2} ")
      ) +
      geom_line(
        data = fitted_data,
        aes(x = f, y = lf_amp),
  ...
  plot_grid(sig_plot, fft_plot, spec_plot, nrow = 3)
}
...
column(width = 3, numericInput("fs", "freq-shift[Hz/s]", value = 2)),
...
renderPlot({summary_plot(sig_data(),  spec_data(), input$bp, f0 = input$f0_start, d = 2 * input$d_start)})
