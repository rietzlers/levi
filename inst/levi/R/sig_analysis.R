...
signalAnalysis <- function(input, output, session, tevi_model, sample_specs, tasks, notifications){
  ...
  # module-calls --------
  c(data_selection, signal_name) %<-% callModule(oscillogram_ctrl, "oscillogram", tevi_model)
  parameter_estimates <- callModule(spectrum_ctrl, "spectrum_analysis",
                                    tevi_model, data_selection, signal_name)
